...
def add_data_args(parser):
    ...
    data.add_argument(
        "--rgb-mean", type=str, default="123.68,116.779,103.939", help="a tuple of size 3 for the mean rgb"
    )
    data.add_argument("--pad-size", type=int, default=0, help="padding the input image")
    data.add_argument("--image-shape", type=str, help="the image shape feed into the network, e.g. (3,224,224)")
    data.add_argument("--num-classes", type=int, help="the number of classes")
    data.add_argument("--num-examples", type=int, help="the number of training examples")
    data.add_argument("--data-nthreads", type=int, default=4, help="number of threads for data decoding")
    data.add_argument("--benchmark", type=int, default=0, help="if 1, then feed the network with synthetic data")
    return data
...
def add_data_aug_args(parser):
    ...
    aug.add_argument(
        "--max-random-aspect-ratio", type=float, default=0, help="max change of aspect ratio, whose range is [0, 1]"
    )
    aug.add_argument(
        "--max-random-rotate-angle", type=int, default=0, help="max angle to rotate, whose range is [0, 360]"
    )
    aug.add_argument(
        "--max-random-shear-ratio", type=float, default=0, help="max ratio to shear, whose range is [0, 1]"
    )
